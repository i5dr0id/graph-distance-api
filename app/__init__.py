...
def similarity_score():
	...
	matchedge_filtered_new = matchedge_filtered[:]
	SEDStartTimer = time.time()
	roundFigure = 4
	for se in sourceedge_filtered:
		es = se[::-1]
		if (se in matchedge_filtered_new):
			matchedge_filtered_new.remove(se)
			sourceedge_filtered_new.remove(se)
		elif (es in matchedge_filtered_new):
			matchedge_filtered_new.remove(es)
			sourceedge_filtered_new.remove(se)
		else:
			continue
	if (len(sourceedge_filtered) == 0 and len(matchedge_filtered) == 0):
		...
		SEDStopTimer = time.time()
		SEDtotalTimer = abs(SEDStartTimer - SEDStopTimer)
